...
export const cleanupExpiredQueues = async (
  ...
  try {
    ...
    const deletedResult = await prisma.queue.deleteMany({
      where: {
        createdAt: {
          lt: cutoffDate,
        },
...
const scheduleQueueCleanup = (): void => {
  const taskFunction = async () => {
    ...
    const result = await cleanupExpiredQueues(1); // Hapus data lebih dari 1
    console.log("Cleanup results:", result);
  };

  const task = createCronTask("0 2 * * *", taskFunction);
  jobs.set("queueCleanup", task);
  console.log("Queue cleanup job schedule: Every day at 02:00 (Asia/Jakarta)");
};
